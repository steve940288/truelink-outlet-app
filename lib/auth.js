...
function getSecret() {
  ...
  if (!secret) {
    throw new Error(
      "SESSION_SECRET environment variable is not set. Generate one (e.g. `openssl rand -hex 32`) and set it in Vercel's project settings."
    );
  }
  return secret;
}
...
/**
 * Given a request's session cookie, returns { username, role } if valid,
 * or null. Use this in API routes that need role-based checks (dashboard,
 * edit, export) rather than just verifySessionToken alone.
 */
export function getSessionUser(request) {
  ...
  if (!user) return null;
  return { username: session.username, role: user.role || "staff", displayName: user.displayName };
}

export const DASHBOARD_ROLES = ["admin", "dashboard"];
